...
events ([http://psych.cornell.edu/sites/default/files/Gilo.Vallone.Tversky.pdf]
(http://psych.cornell.edu/sites/default/files/Gilo.Vallone.Tversky.pdf)). This 
paper started a great controversy that continues to this day, as you can 
...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    A streak length of 1 means only one hit then a miss while a streak length of zero (0) means no hits, just a miss.
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
        ...
        Kobe's streak length was right skewed and unimodal, with a long right tail.
        
   What was his typical streak length? How long was his longest streak of baskets?
   
        The typical streak length was a zero (0) having the most entries. The longest streak length was 4.
...
```{r sim-unfair-coin_table, eval=TRUE}
...
load library mosaic set eval = TRUE and include = FALSE to prevent printout.
install.packages("mosaic")
library(mosaic)

```{r, load data and libraries, include= FALSE, eval = TRUE}
#install.packages("mosaic")
library(mosaic)
```

```{r calc-streak-sim_basket, eval=TRUE}
...
-   Describe the distribution of streak lengths. What is the typical streak 
    ...
    The independent shooter has a typical shoting streak of zero (0) and one. The streak distribution is unimodal having only one prominent peak and is skewed to the right with a         long right tail.
    
   How long is the player's longest streak of baskets in 133 shots?
   
    The longest streak is 7.
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    Running the simulation for the independent sahooter a second time would yield somewhat similar results since this is a randon simulation. We have set the seed value with                 set.seed(76) to ensure replicable pseudorandom results, for replicability of out test. 
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    Both distributions are similar both being unimodal and are skewed to the right.
    
   Using this comparison, do you have evidence that the hot hand model fits Kobe's 
   shooting patterns? Explain.
    
    There is no evidence to support that the hot hand model fits kobe's shooting patterns since both models are somewhat similar to those of an independ shooter.
    
    
```{r, streak lengths, include=TRUE}
kobe_streak <- calc_streak(kobe$basket)
