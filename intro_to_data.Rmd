...
1.  How many cases are there in this data set?  How many variables?  For each 
    ...
    There are 20,000 cases and 9 variables.
    
    The variables are of data type:
...
```{r summary_various_cdc, eval=TRUE}    
summary(cdc$height)
...
3.  What does the mosaic plot reveal about smoking habits and gender?
        ...
        The smoking rate is higher in males than in females.
...
Too much into the Rmd file, thus i have modified the code to ensure it will execute but not print out.

```{r 6th-column, include=FALSE}
cdc[,6]
```
...
First, consider expressions like
        ...
        Note: i modified the code to only print out first 20 rows
...
4.  What does this box plot show? 
        ...
        A relationship definitely exists between BMI and the redfported health status.
        
Pick another categorical variable from the data set and see how it relates to BMI.
List the variable you chose, why you might think it would have a relationship to BMI,
and indicate what the figure seems to suggest. 

Those who excercise seem to have a lower BMI, although not as much as would be expected.

```{r boxplot-bmi_hlthplan, eval=TRUE}
bmi <- (cdc$weight / cdc$height^2) * 703
...
How do these two histograms compare?
        ...
        There is better view of the distribution with breaks.
...
```{r scatterplot-weight-wtdesire, eval=TRUE}
library(ggplot2)

ggplot(cdc,aes(wtdesire,weight)) + geom_point(alpha=1/5)
```
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    wtdiff is a numerical variable resulting from transformation of the original data. 0 means that the person is curretly at their desired weight, a positive means that thet are below their desired weight and have to add on more weight while a negative means that they are currently above their desired weight and will have to lose some of it.
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    Most people want to lose their weight.
    the data is left skewed, with a few outliers.
    the median is -10 showing that on average people want to lose 10pounds,
    
```{r wdiff-summary, eval=TRUE}
summary(wdiff)
boxplot(wdiff)
hist(wdiff,xlab="wt-diff",ylab="total individuals",main="",labels=TRUE)
...
```{r wdiff-by-gender, eval=TRUE}
wdiff_by_gender <- data.frame(wdiff = wdiff,gender = cdc$gender)
boxplot(wdiff ~ gender,data=wdiff_by_gender)
```
